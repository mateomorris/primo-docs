// Breadcrumbs - Updated October 18, 2023
function noop() { }
const identity = x => x;
function run(fn) {
    return fn();
...
// (74:10) {#if i === breadcrumbs.length - 1}
...
// (72:6) {#each breadcrumbs as { url, name }
function create_each_block(ctx) {
	let li;
	let t;

	function select_block_type(ctx, dirty) {
		if (/*i*/ ctx[7] === /*breadcrumbs*/ ctx[0].length - 1) return create_if_block_1;
...
function create_fragment(ctx) {
	let div;
	let if_block = /*breadcrumbs*/ ctx[0].length > 0 && create_if_block(ctx);
	...
	return {
		...
		h() {
			attr(div, "class", "section-container svelte-d3qrpf");
		},
		m(target, anchor) {
