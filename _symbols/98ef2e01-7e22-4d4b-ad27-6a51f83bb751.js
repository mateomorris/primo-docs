// Content - Updated October 18, 2023
function noop() { }
function run(fn) {
    return fn();
}
...
function instance($$self, $$props, $$invalidate) {
				...
				if (button) {
					...
					button.onclick = () => {
						fetch(download_url).then(response => response.blob()).then(blob => {
							fileSaver.saveAs(blob, "block.json"); // get the response as a Blob
	...
	return [content, content_node, props, div0_binding];
}

class Component extends SvelteComponent {
	constructor(options) {
		super();
		init(this, options, instance, create_fragment, safe_not_equal, { props: 2, content: 0 });
	}
}
